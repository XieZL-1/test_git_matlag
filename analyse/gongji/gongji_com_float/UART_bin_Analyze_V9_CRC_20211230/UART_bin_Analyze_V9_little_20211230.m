...
FileName= 'G:\data\UART_Data\20220111\422_scha\JLY52.bin';%
...
%                 round(mod(data_temp4,2^15)), round(mod(data_temp4,2^16))]);
    ...
    RawData_table.Properties.VariableNames ={'IMU_ACC_x', 'IMU_ACC_y', 'IMU_ACC_z', 'IMU_Gyro_x', 'IMU_Gyro_y', ...%每行五个，便于计数
        ...
        'Array_49','Array_50','Array_51','Array_52','Array_53',...
        'Array_54','Array_55','Array_56','Array_57','Array_58',...
        'Array_59','GNSS_Delay_ms_u16','PPS_Count_u32','Timmer_add_Count_u16','Array_63',...
        'Array_64','Vehicle_Speed_Ava','Array_66','Array_67','Array_68','Array_69','Array_70','Array_71'};
...
%                 round(mod(data_temp4,2^15)), round(mod(data_temp4,2^16))]);
    ...
    RawData_table.Properties.VariableNames ={'IMU_ACC_x', 'IMU_ACC_y', 'IMU_ACC_z', 'IMU_Gyro_x', 'IMU_Gyro_y', ...%每行五个，便于计数
        ...
        'Array_54','Array_55','Array_56','Array_57','Array_58',...
        'Array_59','GNSS_Delay_ms_u16','PPS_Count_u32','Timmer_add_Count_u16','Array_63',...
        'Array_64','Vehicle_Speed_Ava','Array_66','Array_67','Array_68','Array_69','Array_70','Array_71','Local_Date','Local_Time','Local_Msec'};
    ...
    if CRC_test == 1
        ...
        %         temp_crc = dec2hex(crc_32(i,:),2);
        %         CRC_32 = strcat(temp_crc(4,:),temp_crc(3,:),temp_crc(2,:),temp_crc(1,:));
        %         temp_crc = hex2dec(CRC_32);
        % %         bool = CRC_mex_he(data(:,i),temp_crc);
        % %         if bool == 0
        % %             RawData_table(i-error,:) = [];%错误数据删除
        % %             error = error+1;
        % %         end
        %
        %     end
...
%% IMU 数据处理b
RawData_table.IMU_ACC_x_ASM330 = RawData_table.Array_66;
RawData_table.IMU_ACC_y_ASM330 = RawData_table.Array_67;
RawData_table.IMU_ACC_z_ASM330 = RawData_table.Array_68;
RawData_table.IMU_Gyro_x_ASM330 = RawData_table.Array_69;
RawData_table.IMU_Gyro_y_ASM330 = RawData_table.Array_70;
RawData_table.IMU_Gyro_z_ASM330 = RawData_table.Array_71;
...
% 局部位置计算(立体)
% [Pos_Local_GNSS] = llh2local_V2([GNSS_Lati,GNSS_Longi,ones(size(GNSS_Lati))],[31.28,121.2,0]);
% [Pos_Local_INS] = llh2local_V2([GNSS_Lati_Fusion,GNSS_Longi_Fusion,ones(size(GNSS_Lati_Fusion))],[31.28,121.2,0]);
% f_figure(103,'局部坐标') %画经纬度图
% hold on
% grid on
% plot3(Pos_Local_GNSS(:,1),Pos_Local_GNSS(:,2),Time,'.','DisplayName','GPS');
% plot3(Pos_Local_INS(:,1),Pos_Local_INS(:,2),Time,'.','DisplayName','Fusion');
% legend
% xlabel('x m')
% ylabel('y m')
