...
% kvh
KVH_sych_enable = 1;
load('G:\data\KVH1750\Rover\20220303\data1\Data_BMAW18200029H_1_new.mat')    % kvh实时数据，

% gongji
filename = 'G:\data\UART_Data\20220303\Gongji\STM32_20220303093645.mat';
load(filename)
...
if KVH_sych_enable ==1
    T_error_a_kvhonline = T_error_a;
    T_error_min_kvhonline = 0;
    err_g_min_kvhonline = inf;
    for i = 1:length(T_error_a_kvhonline)
        T_error_kvhonline = T_error_a_kvhonline(i);
        if mod(i/length(T_error_a_kvhonline),0.2)<= 1/length(T_error_a_kvhonline)
            disp(T_error_kvhonline);
        end
        ...
        T_RAWIMU = KVH_RAWIMU_time - GNSS_postime_gongji(temp_2)/1000 + Time(temp_2) - T_error_kvhonline;
        
        Time_temp = round(T_RAWIMU*1000);
        T_RAWIMU = Time_temp *0.001;     %消除时间精度误差
        %% kvh INSPVAXA 时间同步
        temp = find(RawData_table.Pos_time>0,1);
        KVH_INSPVAXA_time = KVH1750.INSPVA.time_INSPVAXA;
        
        temp_1 = find( KVH_INSPVAXA_time >= GNSS_postime_gongji(temp(1))/1000,1);
        temp_2 = find( abs(GNSS_postime_gongji(temp(1):end)/1000 -KVH_INSPVAXA_time(temp_1)) == min(abs(GNSS_postime_gongji(temp(1):end)/1000 -KVH_INSPVAXA_time(temp_1))) ,1,'last'  ) + temp(1)-1;
        
        T_INSPVAXA = KVH_INSPVAXA_time - GNSS_postime_gongji(temp_2)/1000 + Time(temp_2) - T_error_kvhonline;
        
        Time_temp = round(T_INSPVAXA*1000);
        T_INSPVAXA = Time_temp *0.001;     %消除时间精度误差
        %% 用simulink进行时间同步并存储数据
        
        Sim_T = Time(end);
        sim('time_gongji_KVHonline.slx',Sim_T)
        ...
        if err_g< err_g_min_kvhonline
            err_g_min_kvhonline = err_g;
            T_error_min_kvhonline = T_error_a_kvhonline(i);
            ...
            RawData_table.KVH_Pos_N_sigma = KVH_gongji.signals.values(:,22);
            RawData_table.KVH_Pos_E_sigma = KVH_gongji.signals.values(:,23);
    ...
    disp('kvhonline final')
    disp(err_g_min_kvhonline);
    disp(T_error_min_kvhonline);
    ...
    T_RAWIMU = KVH_RAWIMU_time - GNSS_postime_gongji(temp_2)/1000 + Time(temp_2) - T_error_min_kvhonline;
    ...
    T_INSPVAXA = KVH_INSPVAXA_time - GNSS_postime_gongji(temp_2)/1000 + Time(temp_2) - T_error_min_kvhonline;
...
if KVH_sych_enable ==1
    save_name = [save_name, '_kvhonline'];
end
if daoyuan_sych_enable ==1
